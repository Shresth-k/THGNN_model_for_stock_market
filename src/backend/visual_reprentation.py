...
def evaluate_model(model, test_data, ticker_to_idx, edge_index, edge_weight, device='cuda'):
    ...
    # For direction accuracy, we need to know if the prediction and target moved in the same direction
    # Since we're working with normalized data, positive values indicate upward movement
    # and negative values indicate downward movement
    direction_correct = np.sum((predictions > 0) == (targets > 0))
    direction_accuracy = direction_correct / len(predictions) * 100
    
    # Calculate threshold accuracy
    # Percentage of predictions within 5% of the target value
    threshold = 0.05  # 5% threshold
    within_threshold = np.sum(np.abs(predictions - targets) <= threshold * np.abs(targets))
    threshold_accuracy = within_threshold / len(predictions) * 100
    ...
    for idx in np.unique(tickers):
        ...
        # Calculate stock-specific direction accuracy
        stock_dir_correct = np.sum((stock_preds > 0) == (stock_targets > 0))
        stock_dir_accuracy = stock_dir_correct / len(stock_preds) * 100 if len(stock_preds) > 0 else 0
        
        stock_metrics[idx_to_ticker[idx]] = {
            'rmse': stock_rmse,
            'mae': stock_mae,
            'direction_accuracy': stock_dir_accuracy
        }
...
def main():
    ...
    # Print evaluation metrics including the new accuracy metrics
    print(f"Test RMSE: {evaluation['rmse']:.4f}")
    print(f"Test MAE: {evaluation['mae']:.4f}")
    print(f"Test R²: {evaluation['r2']:.4f}")
    print(f"Direction Accuracy: {evaluation['direction_accuracy']:.2f}%")
    print(f"Threshold Accuracy (5%): {evaluation['threshold_accuracy']:.2f}%")
...
def visualize_training(history):
    ...
    # Accuracy metrics
    plt.subplot(2, 2, 4)
    if 'val_direction_accuracy' in history:
        plt.plot(history['val_direction_accuracy'], label='Direction Accuracy', color='purple')
    if 'val_threshold_accuracy' in history:
        plt.plot(history['val_threshold_accuracy'], label='Threshold Accuracy', color='orange')
